...
17. At the Site Bindings menu, add in the HTTPS binding. Click "Add" and within the Add Site Binding window, select the type to be HTTPS. Next click on the dropdown menu below SSL certificate and select the Web Server SSL cert that was created via the same "Friendly name" that was set as an identifier (Different friendly name due to having done this before taking notes)  
