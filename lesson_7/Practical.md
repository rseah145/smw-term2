...
18. On the Win10 client, check that the newly created HTTPS binding on server1 works by browsing to https://<FQDN of server1\>/webdoc note that there is no warning message before accessing the page. The padlock icon beside the URL shows that the browser has accepted the SSL cert  
