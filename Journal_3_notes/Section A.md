...
7. The file server uses its own password to decrypt the ticket sent by the client. (If successful, authenticates and verifies that the ticket was created by the KDC the only other, otherwise it fails to authenticate)  
